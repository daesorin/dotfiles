...
if command -v bat &> /dev/null; then
    ...
    alias catt='bat --style=numbers,changes' # Show line numbers and git changes
fi

# Ripgrep (Grep replacement)
if command -v rg &> /dev/null; then
    alias grep='rg'
    alias rgg='rg -S' # Smart case
else
    # Fallback coloring
...
if command -v eza &> /dev/null; then
    ...
    _EZA_BASE="eza --group-directories-first --icons --color=always --git"
